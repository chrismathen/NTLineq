...
if __name__ == '__main__':
    ...
    def data_list():
        for i in range(len(landscan_pop_path)):
            data=dict()
            filenames_landscan=landscan_pop_path[i][landscan_pop_path[i].find("n-")+2 : landscan_pop_path[i].find(".tif")].replace("global-","pop")
            
            data["file_name"]=filenames_landscan
            data['file_path']=landscan_pop_path[i]
    ...
    with Pool(4) as pool:
        ...
        # for data in list(data_dict.values())[:1]:
            # result=poly2(data)
            # df_hr_1km
            df_hr_1km = gpd.read_file(fr"outputpath\{km}km\india_level6_city_{km}km.shp")
            df_hr_1km['pop_ld_'+result['file_name']] = result['ld']
            ctr+=1
            print(f"{ctr} - Done for {result['file_name']}")
            df_hr_20kmcsv = pd.DataFrame(df_hr_1km)
            df_hr_20kmcsv.to_csv(f"output/pop_{result['file_name']}_df_hr_1km_ntl_final.csv", index = False)
    pass